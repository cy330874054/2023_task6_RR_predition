...
class BertHSLN(torch.nn.Module):
    ...
    def __init__(self, config, tasks):
        ...
        # self.word_attention = nn.TransformerEncoderLayer(
        #     d_model=self.bert.bert_hidden_size, nhead=8
        # )
        ...
        # self.word_lstm = nn.TransformerEncoderLayer(
        #     d_model=self.bert.bert_hidden_size, nhead=8
        # )
        # self.word_lstm = nn.TransformerEncoder(self.word_lstm,num_layers=2)
        ...
        # self.attention_pooling = AttentionPooling(self.lstm_hidden_size ,
        #                                           dimension_context_vector_u=config["att_pooling_dim_ctx"],
        #                                           number_context_vectors=config["att_pooling_num_ctx"])
        ...
        #self.init_sentence_transformers_encode(config,tasks)
        self.reinit_output_layer(tasks, config)


    def init_sentence_transformers_encode(self,config,tasks):
        input_dim = 768
        self.sentence_encoder = nn.TransformerEncoderLayer(
            d_model=input_dim, dim_feedforward=2*input_dim, nhead=8
        )
        self.sentence_encoder = nn.TransformerEncoder(self.sentence_encoder, num_layers=2)

    @staticmethod
    def get_key_padding_mask(tokens):
        key_padding_mask = torch.zeros(tokens.size())
        key_padding_mask[tokens == 0] = 1
    ...
    def forward(self, batch, labels=None, output_all_tasks=False):
            ...
            # bert_embeddings_encoded = bert_embeddings.permute(1, 0, 2)

            bert_embeddings_encoded = self.word_lstm(bert_embeddings,tokens_mask)

            # bert_embeddings_encoded = bert_embeddings_encoded.transpose(0, 1)
            ...
            # sentence_embeddings_encoded = self.sentence_encoder(sentence_embeddings)
            # sentence_embeddings_encoded = sentence_embeddings_encoded.transpose(0, 1)
